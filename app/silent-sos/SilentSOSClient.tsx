...
const OFFLINE_GUARDIANS_KEY =
  "myogun_offline_guardians";

type OfflineGuardianContact = {
  id: string;
  name: string;
  phone: string;
};
...
export default function SilentSOSClient() {
  ...
  const [status, setStatus] =
    useState<EmergencyStatus>("READY");
  ...
  const [timeline, setTimeline] =
    useState<EmergencyTimelineItem[]>([]);
  ...
  const [offlineGuardians, setOfflineGuardians] =
    useState<OfflineGuardianContact[]>([]);
  ...
  const engineRef =
    useRef<EmergencyEngine | null>(null);

  const evidenceRef =
    useRef<EvidenceEngine | null>(null);
              ...
              setTimeline((old) => [
                {
                  time: new Date().toLocaleTimeString(),
                  message:
                    "Initial SOS location evidence saved.",
                },
                ...old,
              ]);
            } catch {
              setTimeline((old) => [
                {
                  time: new Date().toLocaleTimeString(),
                  message:
                    "Evidence queued for later upload.",
                },
                ...old,
              ]);
            }
  ...
  useEffect(() => {
    function loadCachedGuardians() {
      try {
        ...
        console.error(
          "Load cached guardians error:",
          error
        );
      }
    }

    async function refreshGuardianCache() {
      if (!navigator.onLine) {
        return;
      }
  ...
  const cachedGuardian =
    offlineGuardians.find(
      (guardian) =>
        Boolean(guardian.phone)
    );

  const emergencyGuardian = liveGuardian
    ? {
        name: liveGuardian.guardianName,
        phone: liveGuardian.guardianPhone,
      }
    : cachedGuardian
      ? {
          name: cachedGuardian.name,
          phone: cachedGuardian.phone,
        }
      : null;
      ...
      setTimeline((old) => [
        {
          time: new Date().toLocaleTimeString(),
          message:
            "Unable to stop SOS. Please try again.",
        },
        ...old,
      ]);
    }
        ...
        !isOnline && (
          <section className="mt-6 rounded-[2rem] border border-orange-500/40 bg-orange-500/10 p-5">
            ...
            {emergencyGuardian && (
              <div className="mt-4 rounded-2xl border border-white/10 bg-black/30 p-4">
                ...
                <a
                  ...
                  className="mt-4 block w-full rounded-full bg-white px-5 py-3 text-center font-black text-black"
                >
                  📞 Call Guardian
                </a>
              </div>
            ...
            <p className="mt-3 text-center text-xs text-white/50">
              {emergencyGuardian
                ? `SMS will be prepared for ${emergencyGuardian.name}.`
                : "No cached Guardian is available for offline contact."}
            </p>
          </section>
        )}
